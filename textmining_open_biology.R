...
###########################anonymous, Version, recommendation, days and word count###################################
    ...
    comment <- vector()     #Vector that contains comments to the authors
    ...
    for (i in 1:length(lines)){
      ...
      if (switch == 1) {
        if (comment[x] == "")
          ...
          comment[x] <- paste(comment[x], lines[i], sep="\n")
...
##############################################Data frame #####################################################
    if (length(word_count>=1)){
      for (i in 1:length(word_count + 1)){
        ...
        df_comment <- append(df_comment, comment[i])
      }
    }
  }
}#end all_loop

df <- data.frame(df_link, df_section, df_days, df_version, df_recommendation, df_word_count, df_anonymous, df_reviewer_name, df_reviewer_number, df_comment)
